...
export function StructuredResultsView({
  ...
  return (
    <div className="w-full space-y-6">
      ...
      {isLowDetection && (
        <div className="border border-subtle bg-background rounded p-3.5 text-xs text-secondary flex items-start gap-2.5">
            ...
            {summaryTags.map((tag) => {
              ...
              return (
                ...
                >
                  ...
                  {isCaution && <AlertTriangle className="w-3.5 h-3.5 text-amber-700" />}
                  <span>{tag.label}</span>
              ...
              {nutrientRows.map((row) => {
                ...
                return (
                  ...
                  >
                    <span
                      className={`${
                        row.label === "Calories" || row.label === "Total Fat" || row.label === "Total Carbohydrate" || row.label === "Protein"
      ...
      {/* 3. Collapsible Raw OCR Text Section */}
      <div className="border border-subtle bg-surface rounded-md">
        <details className="group">
          <summary className="px-4 py-3 text-xs font-medium text-secondary hover:text-primary cursor-pointer select-none list-none flex items-center justify-between">
            <div className="flex items-center gap-2">
              <FileText className="w-3.5 h-3.5" />
              <span>View raw extracted OCR text ({ocrResult.text.length} characters)</span>
            </div>
            <ChevronDown className="w-3.5 h-3.5 text-secondary group-open:rotate-180 transition-transform" />
          </summary>
          ...
          <div className="p-4 pt-0 space-y-3 border-t border-subtle/50 mt-1">
            <div className="flex items-center justify-between pt-2">
              ...
              <button
                ...
                className="text-xs font-medium text-primary hover:text-accent transition-colors inline-flex items-center gap-1.5 py-1 px-2.5 border border-subtle rounded bg-background hover:bg-subtle"
              >
                {copied ? (
                  <>
            ...
            <pre
              ...
              tabIndex={0}
              className="w-full bg-background border border-subtle rounded p-3.5 font-mono text-xs text-primary leading-relaxed whitespace-pre-wrap break-words max-h-[300px] overflow-y-auto selection:bg-accent/20 focus:outline-none"
            >
              {ocrResult.text}
            </pre>
